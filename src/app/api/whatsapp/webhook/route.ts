 ...
 * resto (reaction, desconhecidos) cai em texto.
 */
function mapMetaTypeToContentType(metaType: string): InboundContentType {
  const allowed = new Set<InboundContentType>([
    'text',
    'image',
    'document',
    'audio',
  ...
  if (allowed.has(metaType as InboundContentType)) {
    return metaType as InboundContentType
  }
...
/**
 * Traduz uma mensagem da Meta para o formato normalizado e delega a
 * regra de negócio para `processInboundMessage`.
 *
 * Fica aqui, e nao em inbound.ts, tudo que e especifico da Meta: o
 * formato do payload, o proxy de midia autenticado e as reacoes (que a
 * Meta entrega como um tipo de mensagem, mas que nao sao mensagens).
 */
async function processMetaMessage(
  ...
  // Reactions short-circuit before the common path — they are not
  // messages: no row in `messages`, no unread bump, no
  // last_message_text. Resolving contact + conversation first is
  // needed because the reaction is keyed on the conversation.
  if (message.type === 'reaction') {
    ...
    // A conversa recem-aberta por uma reacao ainda emite o evento.
  ...
  // Parse message content based on type (Meta-specific: media needs an
  // authenticated verification call before we can build the proxy URL).
