...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'Nenhuma linha válida encontrada. Verifique se o CSV tem uma coluna "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (tagsAssigned > 0) {
        toast.success(
          `${tagsAssigned} atribuiç${tagsAssigned !== 1 ? 'ões' : 'ão'} de tag aplicada${tagsAssigned !== 1 ? 's' : ''}`
        );
      }
      if (skippedNames.length > 0) {
        ...
        toast.info(
          `Tags desconhecidas ignoradas (crie-as em Configurações primeiro): ${sample}${more}`
        );
      }
      if (skipped > 0) {
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              ...
              (separadas por vírgula; use aspas em células com múltiplas tags).
            </DialogDescription>
          </DialogHeader>
